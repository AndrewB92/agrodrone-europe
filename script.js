...
const normalizeGermanFooter = () => {
  ...
  const createFooterColumn = (title, links) => {
    ...
    links.forEach(({ href, label, current }) => {
  ...
  const isImpressum = window.location.pathname.startsWith("/impressum");

  columns[1].replaceWith(
  ...
  columns[2].replaceWith(
    createFooterColumn("Rechtliches", [
      {
        href: "/impressum/",
        label: "Impressum",
        current: isImpressum,
